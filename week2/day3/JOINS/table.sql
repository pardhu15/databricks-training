...
Vijay IT
Vinay IT
• Note: Only employees with matching dept_id in the departments table are included.

2. LEFT OUTER JOIN (LEFT JOIN)
A LEFT JOIN returns all rows from the left table (employees), and matching rows from the right
table (departments). If no match is found, NULL is returned for the right table.
Example:
SELECT e.emp_name, d.dept_name
FROM employees e
LEFT JOIN departments d ON e.dept_id = d.dept_id;
Explanation:
This query retrieves all employees and their corresponding department names. If an employee
doesn't belong to any department, NULL is returned for the department.
...
Vinay IT
Meena NULL
• Note: Meena is included even though they don't belong to a department (NULL value in
dept_name).

3. RIGHT OUTER JOIN (RIGHT JOIN)
A RIGHT JOIN returns all rows from the right table (departments), and the matching rows from the
left table (employees). If no match is found, NULL is returned for the left table.
Example:
SELECT e.emp_name, d.dept_name
FROM employees e
RIGHT JOIN departments d ON e.dept_id = d.dept_id;
Explanation:
This query retrieves all departments and the corresponding employee names. If no employee
belongs to a department, NULL is returned for the employee.
Sample Data (Same as above):
emp_id emp_name dept_id
1 Karthik 1
2 Ajay 1
...
Vinay IT
NULL Finance
• Note: The Finance department is listed even though no employees are in it (NULL for
emp_name).

4. FULL OUTER JOIN
A FULL OUTER JOIN returns all rows from both tables, combining the results of LEFT JOIN and
RIGHT JOIN. If no match is found, NULL is returned for the missing side.
...
Explanation:
This query retrieves all employees and all departments, even if there is no match. If there is no
matching department for an employee, NULL is returned for dept_name, and vice versa.
Sample Data (Same as above):
emp_id emp_name dept_id
1 Karthik 1
emp_id emp_name dept_id
...
Meena NULL
NULL Finance
• Note: Both Meena (who is not assigned to any department) and the Finance department
(which has no employees) are included.
